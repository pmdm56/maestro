...
int nf_process(uint16_t device, uint8_t **buffer, uint16_t packet_length,
  ...
  if (device == config.wan_device) {
    ...
    // Policer -> just accept incoming packet from wan but not policing
    ...
    dst_device_pol = config.lan_device;
    ...
    // flowmanager Policer -> Police outgoing packets
    ...
    if (device == config.lan_device) {
      ...
      bool fwd = policer_check_tb(rte_ipv4_header->dst_addr, packet_length, now,
                                  dynamic_ft);
      dst_device_pol = fwd ? config.wan_device : device;
    } else
      dst_device_pol = device;
  }
